...
const Catalog: React.FC = () => {
  ...
  const toggleFavorite = async (carId: number, event: React.MouseEvent) => {
    ...
    try {
      if (favoriteCarIds.has(carId)) {
        ...
        await favoriteService.addToFavorites({ userId: user.id, carId });
        setFavoriteCarIds((prev) => new Set(prev).add(carId));
      }
    } catch (error: any) {
              ...
              <div key={car.id} className="car-card" style={{ position: 'relative' }}>
                {user && (
                  <button
                    ...
                    title={favoriteCarIds.has(car.id) ? 'Remove from favorites' : 'Add to favorites'}
                  >
                    {favoriteCarIds.has(car.id) ? '❤️' : '🤍'}
                  </button>
                )}
                <Link 
                  to={`/car/${car.id}`} 
