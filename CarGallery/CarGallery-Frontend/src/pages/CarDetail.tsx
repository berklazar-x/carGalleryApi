...
const CarDetail: React.FC = () => {
  ...
  const checkFavoriteStatus = async () => {
    if (!user || !id) return;
    try {
      const isFav = await favoriteService.checkIsFavorite(user.id, Number(id));
      setIsFavorite(isFav);
    } catch (error) {
  ...
  const toggleFavorite = async () => {
    ...
    try {
      if (isFavorite) {
        await favoriteService.removeFromFavorites(user.id, Number(id));
        setIsFavorite(false);
      } else {
        await favoriteService.addToFavorites({ userId: user.id, carId: Number(id) });
        setIsFavorite(true);
      }
    } catch (error: any) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Save to backend
      await carService.updateImages(car.id, newImageUrls);
          ...
          <div className="action-buttons" style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
            {user && (
              <button 
                ...
                style={{
                  flex: 1,
                  background: isFavorite ? '#ff4444' : '#f0f0f0',
